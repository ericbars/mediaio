 ...
 * Lesser General Public License for more details.Conversion failed
 *
...
// mediaio add
extern int64_t my_duration;
static int64_t my_last_pts = 0;

const char program_name[] = "ffmpeg";
const int program_birth_year = 2000;
static progressCallback *callback_android = NULL;
...
static void ffmpeg_cleanup(int ret)
{
	...
	av_log(NULL, AV_LOG_ERROR, "ffmpeg_cleanup");
    ...
    if (received_sigterm) {
		...
		if(callback_android){
			// -200 is conversion failed!
			callback_android(-200);
		}
    }
    term_exit();
    ffmpeg_exited = 1;
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (av_stream_get_end_pts(ost->st) != AV_NOPTS_VALUE) {
            ...
            if(pts > my_last_pts){
                my_last_pts = pts;
				if(callback_android){
					callback_android(pts*100/my_duration);
				}
                //av_log(NULL, AV_LOG_WARNING, "pts = %lld, my_duration = %lld, %d", pts, my_duration, pts*100/my_duration);
            }
        }

        if (is_last_report) {
            //av_log(NULL, AV_LOG_WARNING, "is last report");
            nb_frames_drop += ost->last_dropped;

			// mediaio add
			my_last_pts = my_duration;
			if(callback_android){
				callback_android(100);
			}
        }
    }
